function getProductsOfAllIntsExceptAtIndex(intArray) {
  ...
  if (intArray.length < 2) throw new Error('Array must have more than 2 integers!');
  const products = [];
  let prevProd = 1;

  for (let i = 0; i < intArray.length - 1; i++) {
    const nextProd = intArray.slice(i + 1).reduce((a, b) => a * b);
    products.push(nextProd * prevProd);
    prevProd *= intArray[i];
  }
  products.push(prevProd);
