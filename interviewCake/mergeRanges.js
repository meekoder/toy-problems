function mergeRanges(meetings) {
  ...
  for (let i = 1; i < sortedMeetings.length; i++) {
    ...
    const lastMergedMeeting = merged[merged.length - 1];
    if (currMeeting['startTime'] <= lastMergedMeeting['endTime']) {
      lastMergedMeeting['endTime'] = Math.max(currMeeting['endTime'], lastMergedMeeting['endTime']);
    } else {
