function isFirstComeFirstServed(takeOutOrders, dineInOrders, servedOrders) {
  ...
  for (let i = 0; i < servedOrders.length; i++) {
    const currOrder = servedOrders[i];
    if (dineInOrders[dineInPointer] !== currOrder && takeOutOrders[takeOutPointer] !== currOrder) return false;
    if (dineInOrders[dineInPointer] === currOrder) {
      dineInPointer++;
    } else if (takeOutOrders[takeOutPointer] === currOrder) {
      takeOutPointer++;
    }
  }
  return takeOutPointer === takeOutOrders.length && dineInPointer === dineInOrders.length;
}
